...
func TestGetApplicationProfile_Success(t *testing.T) {
	profile := ApplicationProfile{ID: 140, Application: ODataRef{ID: 1}, ProfileName: "gpu", HostApplication: ODataRef{ID: 2}}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write(pineWrap(profile))
	}))
	defer srv.Close()
	...
	if got.ProfileName != "gpu" || got.Application.ID != 1 || got.HostApplication.ID != 2 {
